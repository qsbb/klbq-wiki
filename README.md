...
- 传说皮肤自动合并基础形态及各进阶形态
- 查询近期角色生日、公告资讯、当前活动、兑换码、当前赛季及结束时间
- 从 Wiki 随机获取一条喵言喵语
...
| `-<角色>觉醒` | 查询角色觉醒效果与激活消耗，各模式不同时分组展示，支持角色别名 | `-心夏觉醒`、`-奶妈觉醒` |
| `-<角色>语音` | 按语言分组合并转发语音列表（中文在前），每条带序号，文字秒发（可选图片卡片） | `-心夏语音`、`-奶妈语音` |
| `-<角色>语音 <关键词>` | 按关键词筛选语音（台词文本与场景名），单张列表图片 | `-心夏语音 机动` |
| `-<序号>` | 语音查询后收听对应序号语音，时效内仅查询者本人可用，每人独立 | `-1`、`-2` |
| `-地图` | 按模式以图片块展示全部地图名称 | `-地图` |
| `-<地图名>` | 查询地图简介、支持模式、地形图与地图概览 | `-88区` |

角色、武器、角色技能、角色觉醒和地图查询默认返回图片卡片。角色卡会随机展示当前角色的 Wiki 立绘，武器卡会优先展示透明武器图。觉醒卡按游戏模式（爆破/极限推进/弦区争夺）分组展示觉醒效果，各模式内容一致时自动合并，并以蓝/橙/绿色标签标注激活所需的弦能点消耗。
...
| `-礼包码` / `-cdk` | 与 `-兑换码` 相同 |
| `-公告` | 按时间倒序列出近期公告，带序号（序号 1 为最新） |
| `-公告10` | 查看序号 10 的公告详情（支持 `-公告 10` 写法） |
...
-设置 announcement_count 20       # 公告列表显示 20 条
-设置 not_found_reply off         # 查询不到时不再回复提示
...
| `voice_list_image` | 布尔 | `false` | 语音全量列表渲染为图片卡片（默认关闭，文字列表秒发） |
