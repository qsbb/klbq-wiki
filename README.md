...
[![Version](https://img.shields.io/badge/version-1.11.1-5c6ac4)](https://github.com/qsbb/astrbot_plugin_klbq_wiki)
...
| `-当前活动` | 与 `-活动` 相同 |
| `-兑换码` | 以合并消息显示未明确失效的兑换码，每个兑换码单独一条方便复制 |
| `-礼包码` / `-cdk` | 与 `-兑换码` 相同 |
...
| `-更新` / `-强制更新` | 简写形式 |
| `-更新资源` | 预下载全部角色立绘和皮肤图到本地缓存（仅主人，解决图片加载问题） |
