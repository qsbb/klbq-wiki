...
export class ImageCache {
  ...
  _extFromUrl(url) {
    try {
      ...
      const m = pathname.match(/\.(jpg|jpeg|png|gif|webp|bmp|mp3|ogg|wav|m4a)$/)
  ...
  async get(url) {
    ...
    try {
      ...
      if (!resp.ok) {
        log(`[KlbqWiki] 缓存文件下载失败 HTTP ${resp.status}: ${url.slice(0, 100)}`)
        return url
      }
