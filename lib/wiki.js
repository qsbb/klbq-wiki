...
/** 语音文本缓存目录（与图片缓存同在 data/ 下，已被 gitignore） */
const VOICE_CACHE_DIR = path.resolve(process.cwd(), 'plugins', 'klbq-wiki', 'data', 'voices')
...
export class WikiClient {
   ...
   */
  async roleVoices(role, ttlDays = 7) {
    const cached = this._cacheGet(`voices:${role}`)
    if (cached) return cached
    // 本地文件缓存：避免重启或内存过期后重复抓取大体积语音页面
    const disk = this._readVoiceCache(role, ttlDays)
    if (disk) return this._cacheSet(`voices:${role}`, disk)
    ...
    this._writeVoiceCache(role, groups)
    return this._cacheSet(`voices:${role}`, groups)
  }

  /** 语音缓存文件路径（角色名中的文件系统非法字符替换为下划线） */
  _voiceCacheFile(role) {
    const safe = String(role).replace(/[\\/:*?"<>|]/g, '_')
    return path.join(VOICE_CACHE_DIR, `${safe}.json`)
  }

  /** 读取本地语音缓存，超过 TTL（天，0 表示永不过期）视为失效 */
  _readVoiceCache(role, ttlDays) {
    try {
      const file = this._voiceCacheFile(role)
      if (!fs.existsSync(file)) return null
      if (ttlDays > 0) {
        const ageMs = Date.now() - fs.statSync(file).mtimeMs
        if (ageMs > ttlDays * 86400 * 1000) return null
      }
      const data = JSON.parse(fs.readFileSync(file, 'utf8'))
      return Array.isArray(data) && data.length ? data : null
    } catch (err) {
      logger.warn(`[KlbqWiki] 语音缓存读取失败: ${err}`)
      return null
    }
  }

  /** 写入本地语音缓存 */
  _writeVoiceCache(role, groups) {
    try {
      fs.mkdirSync(VOICE_CACHE_DIR, { recursive: true })
      fs.writeFileSync(this._voiceCacheFile(role), JSON.stringify(groups), 'utf8')
    } catch (err) {
      logger.warn(`[KlbqWiki] 语音缓存写入失败: ${err}`)
    }
  }
